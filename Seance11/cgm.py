...
class RandomVariable:
    ...
    def __init__(self,Omega:ProbabilitySpace,domain:range,name:str,data=None,f=None):
        self.Omega = Omega
        self.name = name
        self.f = f # unused
        if data != None:
            ...
            self.data = { omega : random.choice(domain) for omega in self.Omega.all_omegas() }
        else:
            self.data = { omega : f(omega) for omega in self.Omega.all_omegas() }

    def __getitem__(self,omega):
        assert(omega in self.data)
        return self.data[omega]
    
def generate_data_random_function(domain:range,arity:int)->Dict:
    return { t : random.choice(domain) for t in itertools.product(domain,repeat=arity)}
    
    
class RandomVariables:
    
    def __init__(self,Omega:ProbabilitySpace,domain:range,Xs:Tuple[RandomVariable]=None,Ys:Tuple[RandomVariable]=None): 
        ''' 
...
def find_causes_for_joint_distribution(joint_distribution:RandomVariables):
...
class CausalGraphModel:
    ...
    def generate_one_random_variables_model(self,Omega:ProbabilitySpace,domain:range,random_causes=None,debug=False)->RandomVariables:
        # assume that variables are topologicaly sorted
        if random_causes == None:
            random_causes = {}
            for U in self.unobserved_variables:
                random_causes[U] = RandomVariable(Omega,domain,U) # so f=None then generate "random"  random variable on domain
            for O in self.observed_variables:
                random_causes[O] = generate_data_random_function(domain,len(self.causal_functions[O]))
        else:
            pass # do nothing if random_cause is already defined
        if debug:
        ...
        for X in self.variables:
            if X in self.unobserved_variables:
                random_variables[X] = random_causes[X]
            else: # X in self.observed_variables:
                X_data = {} 
                for omega in Omega.all_omegas():
                    parents = tuple([ random_variables[Y][omega] for Y in self.causal_functions[X]])
                    X_data[omega] = random_causes[X][parents]
                random_variables[X] = RandomVariable(Omega,domain,name=X,data=X_data)
        if debug:
           for X in random_variables:
               print(X,random_variables[X].data)
    ...
    def search_empirical_conditional_independances(self,Omega:ProbabilitySpace,domain:range,nb_search=1):
        ...
        for _ in range(nb_search):
            R = self.generate_one_random_variables_model(Omega,domain)
...
#### Mise au point de l'extraction de modele.
...
#         'Y':('Z','T')
#         }
# model = {'X':('Y','Z','T'),'U':('Y','T'),'V':('X','U'),'W':('Y','X','V')}
# model = {'X':('Y','Z')}
# model = {'X':('Y','Z'),'U':('X',)}

# gcm = CausalGraphModel(model)
# gcm.draw()
# print(f'observed {gcm.observed_variables} unobserved {gcm.unobserved_variables}')

# Omega = ProbabilitySpace(100)
# print(dir(Omega))
# domain = list(range(4))
# R = gcm.generate_one_random_variables_model(Omega,domain,debug=True)
 
# R.print_variables()

# for omega in Omega.all_omegas():
#     print(R.Xs[0][omega],R.Xs[1][omega])

# R.print_distributions()


# all_Is = R.list_all_conditional_independances()
# for indep in all_Is:
#     print(indep)

#### Mise au point des independances conditionelle empiriques

# Rs = [ gcm.generate_one_random_variables_model(Omega,domain,debug=True) for _ in range(10)]
# all_Is = set(Rs[0].list_all_conditional_independances())
# for R in Rs[1:]:
#     print('#'*10)
#     Is = R.list_all_conditional_independances()
#     print(set(Is))
#     all_Is = all_Is.intersection(set(Is))
# print('x'*10)
# print(all_Is)

### Small models
# small_models = [{'X':('Y',),'Y':('Z',)},{'X':('Y',),'Z':('Y',)},{'Y':('X','Z')}]
...
# for a_model in small_models:
#     gcm = CausalGraphModel(a_model)
#     its_empirical_conditional_independances = gcm.search_empirical_conditional_independances(Omega,domain,nb_search)
#     print('<Empirical independances>')
#     for i in its_empirical_conditional_independances:
#         print(i)
#     print('</Empirical independances>')
#     # R = gcm.generate_one_random_variables_model(Omega,domain)
#     # R.condition(['Z'])
#     # R.print_distributions()
#     gcm.draw()
...
# model_3_19 = {'Y':('E','F','S'),'E':('A',),'S':('A','X'),'F':('X',),'A':('U',),'X':('U',)}
# nb_search = 5
# Omega = ProbabilitySpace(100)
# domain = range(10)

# gcm = CausalGraphModel(model_3_19)
# gcm.draw()


### Universal explanation of causal graph model
#### "Universal" CGM

# Omega = ProbabilitySpace(100)
# domain = range(10)
# nb_variables = 3
# joint_distribution = generate_random_joint_distribution(Omega,domain,nb_variables)
# joint_distribution.print_distribution()
# find_causes_for_joint_distribution(joint_distribution)
...
### theoretical conditional independances

### comparison empirical and theoretical conditional independances
